...
n\n"
	...
	if [ "$useragent" == "" ];
		then
		useragent="Useragents/Useragent_list.txt"
	fi	
	printf "${WHITE}\n\nWOULD YOU LIKE TO SAVE YOUR LOG SESSIONS(1)YES(2)NO\n\n"
	read -p"$GREEN[#MR.HOLMES#]$WHITE-->" Logs
	while [ "$Logs" == "" ];
...
YOUR DEFAULT CREDENTIALS ARE:\nUSERNAME:${GREEN}Admin\n${WHITE}PASSWORD:${GREEN}Qwerty123\n"
	...
	while [ "$Language" (1) "" ];
		do
		printf "${WHITE}\nSELECT YOUR CLI-DEFAULT LANGUAGE\n(1)ENGLISH\n(2)ITALIANO\n(3)FRANÇAIS\n\n"
		read -p"$GREEN[#MR.HOLMES#]$WHITE-->" Language 1"
	...
	while [ "$DateFormat" == "1" ];
		do
		printf "${WHITE}\nSELECT YOUR DATE-FORMAT\n(1)EUROPE(DD/MM/YYYY)\n(2)AMERICA'USA'(MM/DD/YYYY)\n(3)ASIA(YYYY/MM/DD)\n\n"
		read -p"$GREEN[#MR.HOLMES#]$WHITE-->" DateFormat 2"
	done
	if [ $DateFormat == 1 ];
		then
		Date="eu"
...
function installer
	...
	printf "${GREEN}\n\n[+]${WHITE}LINUX DISTRIBUTION FOUND:$DIST${GREEN}[+]"
	printf "${BLUE}\n\nWELCOME TO THE INSTALLATION MANAGER WOULD YOU LIKE TO BEGIN(1)YES(2)NO\n\n"
	read -p "$GREEN[#MR.HOLMES#]$WHITE-->" confvar (1)
	while [ "$confvar" = "1" ];
		do
		printf "${BLUE}\n\nWELCOME TO THE INSTALLATION MANAGER WOULD YOU LIKE TO BEGIN(1)YES(2)NO\n\n"
		read -p "$GREEN[#MR.HOLMES#]$WHITE-->" confvar
...
'>GUI/Theme/Mode.json
	...
	printf "\n${WHITE}GUI-THEME:${GREEN}$mode\n"
	printf "${WHITE}\nSELECT YOUR GUI-DEFAULT LANGUAGE\n(1)ENGLISH\n(2)ITALIANO\n(3)FRANÇAIS\n\n"
	read -p"$GREEN[#MR.HOLMES#]$WHITE-->" GuiLanguage
	while [ "$GuiLanguage" == "" ];
		do
		printf "${WHITE}\nSELECT YOUR GUI-DEFAULT LANGUAGE\n(1)ENGLISH\n(2)ITALIANO\n(3)FRANÇAIS\n\n"
		read -p"$GREEN[#MR.HOLMES#]$WHITE-->" GuiLanguage
...
printf"\n $WHITEYOUR DEFAULT CREDENTIALS ARE:\nUSERNAME:${GREEN}Admin\n${WHITE}PASSWORD:${GREEN}Qwerty123\n"
	else [ "$Access" == "False" ]
		echo '
	"Database":{
		"Status": "Deactive"
...
'>GUI/Credentials/Login.json	
	...
	echo "" >> Configuration/Configuration.ini
	echo "[Smtp]">>Configuration/Configuration.ini
	echo "status" = "$status">>Configuration/Configuration.ini
	echo "email = $recipient">>Configuration/Configuration.ini

	echo "password = $password">>Configuration/Configuration.ini
	echo "destination = $destination">>Configuration/Configuration.ini
	echo "server= $server">>Configuration/Configuration.ini
	echo "port= $port">>Configuration/Configuration.ini
	echo "" >> Configuration/Configuration.ini
	echo "[Settings]">>Configuration/Configuration.ini	
	echo "password = $up_pass">>Configuration/Configuration.ini
	echo "api_key = $key">>Configuration/Configuration.ini
	echo "proxy_list" = "$proxies">>Configuration/Configuration.ini
	echo "useragent_list" = "$useragent">>Configuration/Configuration.ini
	echo "show_logs = $Logs">>Configuration/Configuration.ini
	echo "database"= "$Token">>Configuration/Configuration.ini
	echo "language"= "$Lang">>Configuration/Configuration.ini
	echo "date_format"= "$Date">>Configuration/Configuration.ini
	rm Configuration/UNTILED.txt &> /dev/null
